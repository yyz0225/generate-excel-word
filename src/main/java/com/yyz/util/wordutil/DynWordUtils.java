...
public class DynWordUtils {
    ...
    public void createWord(String templatePath, String outPath) {
        try (FileOutputStream outStream = new FileOutputStream(outPath);
            ...
            templateDoc = new XWPFDocument(opcPackage);
            parseTemplateWord();
    ...
    private boolean delAndJudgeRow(XWPFTable table, Map<String, Object> paramMap, XWPFTableRow row) throws Exception {
        // table.getRows().get(2).getTableCells().get(1) instanceof XWPFTable true
        // 当前行是动态行标志
    ...
    private List<XWPFTableRow> addAndGetRows(XWPFTable table, XWPFTableRow flagRow, Map<String, Object> paramMap) throws Exception {
        ...
        Map<String,Object> rowMap = getDynText(flagRowCells);

        List<List<String>> dataList = (List<List<String>>) PoiWordUtils.getValueByPlaceholder(paramMap, rowMap.get("dynText").toString());

        boolean singleRowFlag = Boolean.parseBoolean(rowMap.get("singleRowFlag").toString());
        ...
        if (singleRowFlag) {
            // TODO 此处感觉可以写成动态下表,获取动态标识处位置,返回cell下标即可
            ...
            if (flagRowCells != null) {
                for (int i = 0; i < flagRowCells.size(); i++) {
                    XWPFTableCell Cell = flagRowCells.get(i);
                    if (Cell != null) {
                        // 表格里包含特殊处理,也是动态添加行
                        List<IBodyElement> bodyElements= Cell.getBodyElements();
                        // 表格里有多个cell元素,需要遍历判断是否是动态行
                        if (bodyElements.size()>1) {
                            for (int j = 0; j < bodyElements.size(); j++) {
                                ...
                                if (bodyElement instanceof XWPFTable) {
                                    ...
                                    // 此处写死,直接获取表格中第二行的数据,是动态添加行
                                    XWPFTableCell flagCell = xwpfTableRow.getTableCells().get(0);
                                    XWPFTableRow currentRow = xwpfTableRow;
                                    int cellSize = xwpfTableRow.getTableCells().size();
    ...
    /**
     * 获取动态的文本信息
     * @param flagRowCells flagRowCells
     * @return
     */
    private Map<String,Object> getDynText(List<XWPFTableCell> flagRowCells) {
        Map<String,Object> rowMap = new HashMap<>(2);
        String dynText = "";
        boolean singleRowFlag = true;
        if (flagRowCells != null) {
            for (int i = 0; i < flagRowCells.size(); i++) {
                XWPFTableCell cell = flagRowCells.get(i);
                if (cell != null) {
        ...
        rowMap.put("dynText",dynText);
        rowMap.put("singleRowFlag",singleRowFlag);
        return rowMap;
    }
